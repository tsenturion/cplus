...
public:
    std::string render(const std::string& text) override {
        ...
        std::transform(result.begin(), result.end(), result.begin(), ::toupper); // Преобразование в верхний регистр в одной строке
        return result;
    }
};
...
public:
    std::string render(const std::string& text) override {
        std::string result = text;
        std::transform(result.begin(), result.end(), result.begin(), ::tolower); // Преобразование в нижний регистр в одной строке
        return result;
    }
};
...
int main() {
    ...
    delete upperRenderer; //Важно удалять все объекты, созданные через new
    delete lowerRenderer;
